<template>
  <n-card title="" size="large" v-if="this.ready_to_render">
    ...
    <n-button circle style="margin-left: 10px; margin-top: 5px; float: left" @click="trad_simp_conv"> 繁简 </n-button>
    ...
    <div style="max-width: 600px">
      <img src="/DSC09460.jpg" style="max-width: 100%; height: auto; width: auto\9" />
    </div>
  </n-card>
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    clean_data: function () {
      console.log("poem_info_" + this.poem_info_)
    ...
    poetryQuiz() {
      ...
      setTimeout(() => {
        ...
        router.push({ path: "/learn", query: { inputValue: this.poem_info_.p_paragraph } })
      }, 2000)
    }
  },
  mounted() {
    if (this.poem_id !== -1 && this.poem_id !== undefined) {
      axios
        .get(this.BASE_URL + "/query/poem_by_id?p_id=" + this.poem_id)
        .then((response) => {
          this.poem_info_ = response.data
